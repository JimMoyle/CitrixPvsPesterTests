...
If($MSWord -eq $Null)
{
	If($Text -or $HTML -or $PDF -or $Object)
	{
		$MSWord = $False
	}
	Else
...
If($MSWord -eq $False -and $PDF -eq $False -and $Text -eq $False -and $HTML -eq $False -and $Object -eq $False)
{
	$MSWord = $True
}
...
ElseIf($Object)
{
    Write-Verbose "$(Get-Date): Object is set"
}
...
Function ProcessDocumentOutput
{
    ...
    ElseIf($Object)
    {
        $ObjectToReturn = [pscustomobject]@{
            PVSFarmInformation = $script:objectPVSFarmInfo
            Sites = $script:objectSites
            Stores = $script:objectStores
        }
        Write-Output $objectToReturn
    }
...
Function DeviceStatus
{
	...
	If($xDevice -eq $Null -or $xDevice.status -eq "" -or $xDevice.status -eq "0")
	{
        ...
        ElseIf($Object){
            $script:objectvDiskStatus = $xDevice | Select-Object -Property Status
...
#region Initialize variables for Object Output

$objectTaskSchedule = @{}
$objectTaskScripts = @{}
$script:objectConfigureBootstrapSettings = @()
$script:objectServerBootstrap = @()
$script:objectReturn = @()
$script:objectServers = @()
$script:objectStores = @()
$script:objectBootstrapGeneral =@()
$script:objectDiskVersions = @{}
$script:objectVDiskPool = @{}
$script:objectvDiskPersonality = @{}
$script:objectTaskvDisk = @{}
$script:objectUpdateManagementTasks = @{}
$script:objectUpdateManagementvDisks = @{}
$objectPVSFarmInformation = @{}
$script:objectSiteStores = @()
$script:objectCollections = @()
$script:objectSiteHosts = @()
$script:objectStores = @()
$script:objectSites = @()
$objectGeneral = @{}
$objectSecurity = @{}
...
$objectStatus = @{}
$objectProblemReport = @{}
$objectSiteproperties = @{}
$objectGeneralSite = @{}
$objectSecuritySite = @{}
$objectOptionsSite = @{}
$objectvDiskUpdate = @{}
$objectvDiskPool = @{}
$objectvDiskLoadBalancing = @{}
$objectvDiskUpdateManagement = @{}
$objectDeviceCollections = @{}
$objectCollection = @{}
$objectHosts = @{}
$objectFarmViews = @{}
$objectStoresProperties = @{}
$objectStore = @{}
...
Function OutputFarm
{
	...
	If($Script:PVSFullVersion -ge "7.11")
	{
        ...
        ElseIf($Object)
        {
            $objectProblemReport.UserName = $CISUserName
        }
	}

    #Construct PVS Farm Information object to add to return object

    If($Object){
        $script:objectPVSFarmInfo = [pscustomobject]@{
            General = $farm | Select-Object -Property farmName, Description
            Security = Get-PVSAuthGroup -Farm | Select-Object -Property AuthGroupName
            Groups = $authgroups | Select-Object -Property AuthGroupName
            Licensing = $farm | Select-Object -Property LicenseServer, LicenseServerPort, LicenseTradeUp
            Options = $farm | Select-Object -Property AutoAddEnabled, DefaultSiteName, AuditingEnabled, OfflineDatabaseSupportEnabled, @{n='CeipEnabled';e={$ceipResults.Enabled}}
            vDiskVersion = $farm | Select-Object -Property MaxVersions, AutomaticMergeEnabled, MergeMode
            Status = $farm | Select-Object -Property DatabaseServerName, DatabaseInstanceName, DatabaseName, FailoverPartnerServerName, FailoverPartnerInstanceName, MultiSubnetFailover, AdGroupsEnabled
...
Function OutputSite
{
	...
	If($? -and $AuthGroups -ne $Null)
	{
        if($Object)
        {
            $objectSecuritySite = $authgroups | Select-Object AuthGroupName
        }
        else
        {
		    OutputauthGroups $authGroups
	...
	ForEach($Server in $Servers)
	{
		...
		If(Test-Connection -ComputerName $server.servername -quiet -EA 0)
		{
            ...
            ElseIf($Object)
            {
                $objectBootstrapServer = $Server.serverName
            }
			...
			If($? -and $BootstrapNames -ne $Null)
			{
				...
				If($ServerBootstraps.Count -gt 0)
				{
					ForEach($ServerBootstrap in $ServerBootstraps)
					{
                        ...
                        {
                            $objectBootstrap = @{
                                $ServerBootstrap.name = $ServerBootstrap | Select-Object -Property  @{n='bootserver1_Ip';e={$_.bootserver1_Ip.IPAddressToString}},
                                                                                                    bootserver1_Port,
                                                                                                    @{n='bootserver1_Netmask';e={$_.bootserver1_Netmask.IPAddressToString}},
                                                                                                    @{n='bootserver1_Gateway';e={$_.bootserver1_Gateway.IPAddressToString}},
                                                                                                    @{n='bootserver2_Ip';e={$_.bootserver2_Ip.IPAddressToString}},
                                                                                                    bootserver2_Port,
                                                                                                    @{n='bootserver2_Netmask';e={$_.bootserver2_Netmask.IPAddressToString}},
                                                                                                    @{n='bootserver2_Gateway';e={$_.bootserver2_Gateway.IPAddressToString}},
                                                                                                     @{n='bootserver3_Ip';e={$_.bootserver3_Ip.IPAddressToString}},
                                                                                                    bootserver3_Port,
                                                                                                    @{n='bootserver3_Netmask';e={$_.bootserver3_Netmask.IPAddressToString}},
                                                                                                    @{n='bootserver3_Gateway';e={$_.bootserver3_Gateway.IPAddressToString}},
                                                                                                     @{n='bootserver4_Ip';e={$_.bootserver4_Ip.IPAddressToString}},
                                                                                                    bootserver4_Port,
                                                                                                    @{n='bootserver4_Netmask';e={$_.bootserver4_Netmask.IPAddressToString}},
                                                                                                    @{n='bootserver4_Gateway';e={$_.bootserver4_Gateway.IPAddressToString}}
                            }
                            $script:objectBootstrapGeneral += $objectBootstrap
                        }
        ...
        if($object)
        {
            $script:objectConfigureBootstrapSettings += $script:objectServerBootstrap
        }
	...
	If($? -and $Disks -ne $Null)
	{
		ForEach($Disk in $Disks)
		{
            ...
            ElseIf($Object)
            {
                $objectDiskIdentification = $disk | Select-Object -Property Description, Date, Author, Title, InternalName, OriginalFile, HardwareTarget
            }
            ...
            ElseIf($Object)
            {
                $objectDiskVolumeLic =  $disk | Select-Object -Property LicenseMode
            }
			...
			If($? -and $DiskVersions -ne $Null)
			{
				...
				ForEach($DiskVersion in $DiskVersions)
				{
                    ...
                    ElseIf($Object)
                    {
                        $objectDiskVersion = $DiskVersion | Select-Object -Property Version, CreateDate, ScheduledDate, DeviceCount, Access, Type, Description, CanDelete, CanMerge, CanMergeBase, CanPromote, CanRevertTest, CanRevertMaintenance, CanSetScheduledDate, CanOverride, IsPending, GoodInventoryStatus, DiskFileName
                    }
                    if($Object)
                    {
                        $objectVersionToAdd = @{
            ...
            ElseIf($Object)
            {
                $objectDiskLoad = $disk | Select-Object -Property ServerName, RebalanceEnabled, RebalanceTriggerPercent
            }
            ...
            if($object)
            {
                $objectToAdd = [pscustomobject]@{
                    ...
                    Properties = [pscustomobject]@{
                        ...
                        AutoUpdate = $objectDiskAuto
                    }
                    Versions = $script:objectDiskVersions
                    LoadBalancing = $objectDiskLoad
                }
                $script:objectVDiskPool += $objectToAdd
            } #if
		}
	}
	...
	If($? -and $Tasks -ne $Null)
	{
			...
			ForEach($ManagedvDisk in $ManagedvDisks)
			{
				...
				If($? -and $PersonalityStrings -ne $Null)
				{
                    ...
                    ElseIf($Object){
                        ForEach($PersonalityString in $PersonalityStrings.DevicePersonality)
                        {
                            $objectPersonalityToAdd = @{
                                $PersonalityString.Name = $PersonalityString.Value
                            }
				...
				$updateDevice = Get-PvsDiskUpdateDevice -deviceId $ManagedvDisk.deviceId -EA 0 4>$Null
                ...
                if($Object)
                {
                    ...
                    $objectvDiskToAdd = @{
                        "$($ManagedvDisk.storeName)`\$($ManagedvDisk.disklocatorName)" = [ordered]@{
                            ...
                            Personality = $script:objectvDiskPersonality
                            Status = $script:objectvDiskStatus
                            Logging = $script:objectVDiskLogLevel
                        }
                    }

                    $script:objectUpdateManagementvDisks += $objectvDiskToAdd
                }
		...
		If($Tasks -ne $Null)
		{
			...
			ForEach($Task in $Tasks)
			{
                ...
                ElseIf($Object)
                {
                    $objectTaskAccess = $Task | Select-Object -Property PostUpdateApprove
                }

                if ($Object)
                {
                    $objectTaskToAdd = @{
                        $Task.UpdateTaskName = [ordered]@{
                            General = $ObjectTaskGeneral
                            Schedule = $objectTaskSchedule
                            vDisks = $script:objectTaskvDisk
                            ESD = $objectTaskEsd
                            Scripts = $objectTaskScripts
                            Access = $objectTaskAccess
                        }
                    }
                    $script:objectUpdateManagementTasks += $objectTaskToAdd
                }
			} #ForEach($Task in $Tasks)
		}
	}
    ...
    if($Object)
    {
        $objectSiteDiskUpdateManagement = [ordered]@{
            vDisks = $script:objectUpdateManagementvDisks
            Tasks = $script:objectUpdateManagementTasks
        }
    }
	...
	If($? -and $Collections -ne $Null)
	{
		...
		ForEach($Collection in $Collections)
		{
            ...
            ElseIf($Object)
            {
                $objectCollectionGeneral = $Collection | Select-Object -Property CollectionName, Description
            }
            ...
            ElseIf($Object){
                $objectCollectionAutoAdd = $Collection | Select-Object -Property TemplateDeviceName, AutoAddPrefix, AutoAddNumberLength, AutoAddZeroFill, AutoAddSuffix, LastAutoAddDeviceNumber
            }
        ...
        #Construct Object per Collection to output
        If($Object)
        {
            $objectCollectionToAdd = [pscustomobject]@{
                General = $objectCollectionGeneral
                Security = $objectCollectionSecurity
                AutoAdd = $objectCollectionAutoAdd
            }
        }

        $script:objectCollections += $objectCollectionToAdd
...
#>
    ...
    If($Object)
    {
        $SiteHostToAdd = [pscustomobject]@{
            General = $vHosts | Select-Object -Property Type, Name ,Description, VirtualHostingPoolName, Server, Datacenter
            Credentials = $vHosts | Select-Object -Property UserName, Password
            vDiskUpdate = $vHosts | Select-Object -Property UpdateLimit, UpdateTimeout, ShutdownTimeout
        }

        $script:objectSiteHosts += $SiteHostToAdd
    }
    ...
    $objectCurrentSite = [pscustomobject]@{
        SiteName = $PVSSite.SiteName
        Properties = [pscustomobject]@{
            General = $PVSSite | Select-Object -Property Name, Description
            Security = $objectSecuritySite # relies on Get-PVSAuthgroup
            MAK = $PVSSite | Select-Object -Property MakUser, MakPassword
            Options = $PVSSite | Select-Object -Property @{n='AutoAddEnabled';e={$Script:FarmAutoAddEnabled}}, DefaultCollectionName, InventoryFilePollingInterval
            vDiskUpdate = $PVSSite | Select-Object -Property enableDiskUpdate, diskUpdateServerName
        }
        Servers = $script:objectServers # Relies on Get-PVSServer
        ConfigureBootStrapOptions = $script:objectConfigureBootstrapSettings
        vDiskPool = $script:objectVDiskPool
        vDiskUpdateManagement = $objectSiteDiskUpdateManagement
        DeviceCollections = $script:objectCollections
        Hosts = $script:objectSiteHosts
    }
    ...
    $script:objectConfigureBootstrapSettings = @()
    $script:objectVDiskPool = @()
    $objectSiteDiskUpdateManagement = @()
    $script:objectCollections = @()
    $script:objectSiteHosts = @()
	...
	ForEach($Server in $Servers)
	{
		...
		If($? -and $Stores -ne $Null)
		{
			...
			ForEach($store in $stores)
			{
                ...
                ElseIf($Object)
                {
                    $storeToAdd = [pscustomobject]@{
                        $store.storename = [pscustomobject]@{
                            Path = $store.path
                            CachePath = $store.CachePath
                        }
                    }
                    $script:objectSiteStores += $storeToAdd
                }
			}
		...
		If($Script:PVSFullVersion -ge "7.11")
		{
            ...
            ElseIf($Object)
            {
                $objectServerProblemReport = $Server | Select-Object -Property LastBugReportAttempt,LastBugReportSummary, LastBugReportStatus, LastBugReportResult
            }
		}
        ...
        If ($Object)
        {
            $ServerHashtoAdd = [pscustomobject]@{
                Name = $server.ServerName
                General = $server | Select-Object -Property ServerName, Description, PowerRating, EventLoggingEnabled
                Network =  $server | Select-Object -Property @{n='Ip';e={$_.Ip.IpAddressToString}}, FirstPort, LastPort, @{n='ManagementIp';e={$_.ManagementIp.IpAddressToString}}
                Stores = $script:objectSiteStores
                Options = $server | Select-Object -Property AdMaxPasswordAgeEnabled, AdMaxPasswordAge
                ProblemReport = $objectServerProblemReport
                Advanced = [pscustomobject]@{
                    Server = $server | Select-Object -Property threadsPerPort, buffersPerThread, serverCacheTimeout, localConcurrentIoLimit, remoteConcurrentIoLimit
                    Network = $server | Select-Object -Property maxTransmissionUnits, ioBurstSize, nonBlockingIoEnabled
                    Pacing = $server | Select-Object -Property bootPauseSeconds, MaxBootSeconds, maxBootDevicesAllowed, vDiskCreatePacing
                    Device = $server | Select-Object -Property licenseTimeout
    ...
    If($Object)
    {
        $objectStoreToAdd = @{
            General = [pscustomobject]@{
                ...
                FreeSpace = Get-PvsStoreFreeSpace -StoreId $store.StoreId -SiteId $store.SiteId
            }
            ...
            Paths = [pscustomobject]@{
                DefaultStorePath = $store.path
                DefaultWriteCachePaths = $Store.cachePath
            }
        }
        $script:objectStores += $objectStoreToAdd
    }
}
#endregion
